# UI-UX-Experiment3

## Aim:
To Heuristic Evaluation of an Existing Website or App To pick a website and its feature which I will be recreating as a ui ux designer.

## Algorithm:
1. Select and Compare 2-3 Competitor Apps/Websites
  Selected competitors: Flipkart and Meesho.
  
  Compared them with my own website, Hanabana.
  
  Observed their design, navigation, color usage, and content layout.
  
  Noted that Flipkart has a feature-rich interface while Meesho focus on simplicity and low-cost listings.
  
  Found that Hanabana can improve by using clearer category filtering and smoother user flow.

2. Perform Heuristic Evaluation
  Checked Flipkart, Meesho, and Hanabana using usability principles.
  
  Evaluated consistency, visibility, error prevention, and feedback.
  
  Flipkart showed high consistency; Meesho showed easy navigation for budget shoppers.
  
  Hanabana needs better visual hierarchy in the hero section and simpler navigation menu options.

3. Analyze and Document Findings
  Collected all comparison and evaluation data.
  
  Listed strengths and weaknesses of each platform.
  
  Flipkart has strong user experience and fast access to multiple categories.
  
  Hanabana needs improvement in placement of "Shop Now" buttons and product category visibility.

4. Propose and Apply Improvements
  Planned layout changes for Hanabana based on findings.
  
  Simplified navigation and made login/signup buttons more visible.
  
  Used better color contrast and improved font readability for the brand tagline.
  
  Added quick access category options similar to Flipkart’s navigation bar.
  
  Made overall interface cleaner and easier to use for fashion shoppers.
  
## Output:
<img width="1032" height="457" alt="image" src="https://github.com/user-attachments/assets/ed7e542a-f315-4740-8931-82380ec3288c" />
<img width="1056" height="545" alt="image" src="https://github.com/user-attachments/assets/f5c41b52-7fcf-47a4-aed9-7544f0b6e720" />

## Result:
The Heuristic Evaluation of an Existing Website or App is created and verified.
